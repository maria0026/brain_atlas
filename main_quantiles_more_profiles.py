...
def main(args):
    ...
    # powierzchnia fsaverage
    fsavg = datasets.fetch_surf_fsaverage()

    # projekcja objętości → powierzchnia
    texture_left = surface.vol_to_surf(img_transformed, fsavg.pial_left)
    texture_right = surface.vol_to_surf(img_transformed, fsavg.pial_right)
    ...
    for i, (hemi, view) in enumerate(views, 1):
        ax = fig.add_subplot(2, 4, i, projection="3d")
        plotting.plot_surf_stat_map(
            fsavg.infl_left if hemi == "left" else fsavg.infl_right,
