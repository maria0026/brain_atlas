...
class ColorChanger():
    ...
    def read_feature_importance(self):
        ...
        print("Kolumny", df)
        df = df.groupby('feature_name')['mean'].sum().reset_index()
        self.importance_dict = dict(zip(df['feature_name'], df['mean']))
...
def main(args):
    ...
    color_changer_reference = ColorChanger(args.atlas_file, args.LUT_file, args.reference_scores_file)
    img_ref=color_changer_reference.read_atlas()
    color_changer_reference.read_color_LUT()
    color_changer_reference.read_feature_importance()
    importance_map_reference = color_changer_reference.map_feature_importance()
    ...
    vmin = np.min(importance_map_reference)
    vmax = np.max(importance_map_reference)
...
if __name__=="__main__":
    ...
    parser.add_argument(
        '--scores_file', 
        type=str, 
        default='brain_structures_hearing_transformed/hearing_znaczny_trends_M_robust_scaled_degree_2_r2_HF.csv',
